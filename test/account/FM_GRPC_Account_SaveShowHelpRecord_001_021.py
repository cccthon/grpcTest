#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_GRPC_Account_SaveShowHelpRecord_001_021
# 用例标题: 保存获取展示帮助记录
# 预置条件: 
#   1.注册一个账号,记录id信息
# 测试步骤:
#   1.调用接口：SaveShowHelpRecord保存获取展示帮助记录
# 预期结果:
#   1.接口调用成功
# 脚本作者: shencanhui
# 写作日期: 20171019
...
class SaveShowHelpRecord(unittest.TestCase):
    def setUp(self):
        #连接account测试服务器
    ...
    def test_SaveShowHelpRecord(self):
        #保存展示帮助记录
        SaveShowHelpRecord = self.stub.SaveShowHelpRecord(account_pb2.User(Id = self.accountID))
        self.assertEqual(SaveShowHelpRecord.Success, 0)
