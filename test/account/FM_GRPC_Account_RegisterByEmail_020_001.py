#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_GRPC_Account_RegisterByEmail_020_001
# 用例标题: 通过email注册
# 预置条件: 
#   
# 测试步骤:
...
class RegisterByEmail(unittest.TestCase):
    ...
    def test_RegisterByEmail_nullPWD(self):
        ...
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['PasswdInvalid_returnCode'],self.stub.RegisterByEmail,account_pb2.RegisterUserRequest(User = account_pb2.User(AccountEmail = 'testAccount1@test.com')))

    def test_RegisterByEmail_lessPWD(self):
        #注册一个测试账号，超短密码。返回用户无效
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserInvalid_returnCode'],self.stub.RegisterByEmail,account_pb2.RegisterUserRequest(User = account_pb2.User(AccountEmail = 'testAccount2@test.com', UserPassword = 'f')))

    def test_RegisterByEmail_morePWD(self):
        #注册一个测试账号，超长密码。返回用户无效
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserInvalid_returnCode'],self.stub.RegisterByEmail,account_pb2.RegisterUserRequest(User = account_pb2.User(AccountEmail = 'testAccount2@test.com', UserPassword = 'f11111111111111111111111111111111111111111')))
