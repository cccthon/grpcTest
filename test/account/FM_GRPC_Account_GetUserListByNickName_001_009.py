#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_GRPC_Account_GetUserListByNickName_001_009
# 用例标题: 通过昵称获取用户列表信息
# 预置条件: 
#   1.注册一个账号，并设置nickname
# 测试步骤:
#   1.调用接口：GetUserListByNickName
# 预期结果:
#   1.接口调用成功
#   2.参数符合预期
# 脚本作者: shencanhui
# 写作日期: 20171019
#=========================================================
import grpc,sys,unittest,yaml,time,uuid
...
class GetUserListByNickName(unittest.TestCase):
    def setUp(self):
        #连接account测试服务器
    ...
    def test_GetUserListByNickName(self):
        #通过昵称获取用户列表相关信息
        GetUserListByNickName = self.stub.GetUserListByNickName(account_pb2.GetUserListRequest(NickName = userData['NickName']))
        #返回用户列表
        print(GetUserListByNickName)
        # self.assertEqual(GetUserListByNickName.List[0].User.NickName, userData['NickName'])


    # def test_GetCachedUserListByNickName_NotExistName(self):
    #     #查询用户通过不存在的nickName
    #     self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserNotExisted_returnCode'],self.stub.GetUserListByNickName,account_pb2.User(NickName = 'testssssss'))
