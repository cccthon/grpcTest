#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_GRPC_Account_ReloadCachedUser_070_001
# 用例标题: 重新加载用户从缓存中
# 预置条件: 
#   1.注册一个账号,记录id信息
# 测试步骤:
#   1.调用接口：ReloadCachedUser
# 预期结果:
#   1.接口调用成功，返回0
# 脚本作者: shencanhui
# 写作日期: 20171020
...
class ReloadCachedUser(unittest.TestCase):
    def setUp(self):
        #连接account测试服务器
    ...
    def test_ReloadCachedUser(self):
        #重新加载用户从缓存中
        ReloadCachedUser = self.stub.ReloadCachedUser(account_pb2.User(Id = self.accountID))
        self.assertEqual(ReloadCachedUser.Success, 0)
    
    def test_ReloadCachedUser_NotExistID(self):
        #查询用户通过不存在的id
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserNotExisted_returnCode'],self.stub.ReloadCachedUser,account_pb2.User(Id = 999999999))
