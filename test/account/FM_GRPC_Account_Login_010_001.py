...
class Login(unittest.TestCase):
    ...
    def test_Login(self):
        ...
        #登录成功
        Login = self.stub.Login(account_pb2.LoginRequest(User = account_pb2.User(AccountEmail = userData['AccountEmail'],UserPassword = userData['UserPasswd']) ,Token = str(uuid.uuid1())))
    ...
    def test_Login_nullAccount(self):
        ...
        self.assertEqual(validateEmailLink.Success, 0)
        #账号为空，返回无效的用户
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserInvalid_returnCode'], self.stub.Login, account_pb2.LoginRequest(User = account_pb2.User(UserPassword = 'errorPWD') ,Token = str(uuid.uuid1())))

    def test_Login_errorAccount(self):
        #新注册的账号需要先激活才能登录
        #发送验证码邮件到邮箱
        sendActivationEmail = self.stub.SendActivationEmail(account_pb2.SendActivationEmailRequest(UserId = self.accountID,ActivationCode = userData['ActivationCode'],Email = userData['SendActivationEmail'] ,Subject = userData['Subject'] ))
        #断言验证码邮件发送成功，返回0
        ...
        self.assertEqual(validateEmailLink.Success, 0)
        #账号错误，返回无效的用户
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserInvalid_returnCode'], self.stub.Login, account_pb2.LoginRequest(User = account_pb2.User(AccountEmail = 'errorAccount',UserPassword = userData['UserPasswd']) ,Token = str(uuid.uuid1())))
        
    def test_Login_errorAccountPWD(self):
        #新注册的账号需要先激活才能登录
        #发送验证码邮件到邮箱
        sendActivationEmail = self.stub.SendActivationEmail(account_pb2.SendActivationEmailRequest(UserId = self.accountID,ActivationCode = userData['ActivationCode'],Email = userData['SendActivationEmail'] ,Subject = userData['Subject'] ))
        #断言验证码邮件发送成功，返回0
        ...
        self.assertEqual(validateEmailLink.Success, 0)
        #账号密码错误，返回无效的用户
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserInvalid_returnCode'], self.stub.Login, account_pb2.LoginRequest(User = account_pb2.User(AccountEmail = 'errorAccount',UserPassword = 'errorPWD') ,Token = str(uuid.uuid1())))
    ...
    def test_Login_nullPWD(self):
        ...
        self.assertEqual(validateEmailLink.Success, 0)
        #空密码登录，返回密码无效
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['PasswdInvalid_returnCode'], self.stub.Login, account_pb2.LoginRequest(User = account_pb2.User(AccountEmail = userData['AccountEmail']) ,Token = str(uuid.uuid1())))
    ...
    def test_Login_errorPWD(self):
        ...
        self.assertEqual(validateEmailLink.Success, 0)
        #错误密码登录，返回用户不存在
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserNotExisted_returnCode'], self.stub.Login, account_pb2.LoginRequest(User = account_pb2.User(AccountEmail = userData['AccountEmail'],UserPassword = 'errorPWD') ,Token = str(uuid.uuid1())))
