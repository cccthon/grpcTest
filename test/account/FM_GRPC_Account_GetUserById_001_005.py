#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_GRPC_Account_GetUserById_001_005
# 用例标题: 通过id获取用户信息
# 预置条件: 
#   1.注册一个账号,记录id信息
# 测试步骤:
#   1.调用接口：GetUserById，入参为：注册时记录的id
...
class GetUserById(unittest.TestCase):
    def setUp(self):
        #连接account测试服务器
    ...
    def test_GetUserById(self):
        getCachedUser = self.stub.GetUserById(account_pb2.User(Id = self.accountID))
    ...
    def test_GetUserById_NotExistID(self):
        #查询用户通过不存在的id
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserNotExisted_returnCode'],self.stub.GetUserById,account_pb2.User(Id = 999999999))
