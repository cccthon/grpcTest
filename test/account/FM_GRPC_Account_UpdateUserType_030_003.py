#========================================================
#+++++++++++++++++  测试用例信息   ++++++++++++++++
# 用例  ID: FM_GRPC_Account_UpdateUserType_030_003
# 用例标题: 更新用户类型
# 预置条件: 
#   1.注册一个账号,记录id信息
# 测试步骤:
#   1.调用接口：UpdateUserType
# 预期结果:
#   1.接口调用成功，返回0
# 脚本作者: shencanhui
# 写作日期: 20171020
...
class UpdateUserType(unittest.TestCase):
    def setUp(self):
        #连接account测试服务器
    ...
    def test_UpdateUserType(self):
        #更新用户类型，返回成功
        UpdateUserType = self.stub.UpdateUserType(account_pb2.User(Id = self.accountID))
        self.assertEqual(UpdateUserType.Success, 0)
    
    def test_UpdateUserType_NotExistID(self):
        #查询用户通过不存在的id
        self.assertRaisesRegex(grpc._channel._Rendezvous,userData['UserNotExisted_returnCode'],self.stub.UpdateUserType,account_pb2.User(Id = 999999999))
